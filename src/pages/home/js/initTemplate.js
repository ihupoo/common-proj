    ...
    initHomeTemplate: function () {
        getAllModules().then(res => {
            if (res.success && res.data) {
                ...
                this.initSystemModule(modules);
            }
        })
        this.initMainContent(Home.Data.panelData);
    },
    initSystemModule: function (modules) {
        let that = this;
        this.processData = function (data) {
            for (var attr in data) {
                if (Home.Data.sysmode === 'buildSelf' || Home.Data.isSimpleMcu) {//自建下  有权限显示无权限隐藏
                    if (data[attr].value) {
                        data[attr].show = 'show'
                    } else {
                        data[attr].url = '';
                ...
                if (Home.Data.sysmode === 'rent' && !Home.Data.isSimpleMcu) {
                    if (data[attr].value) {
                        data[attr].show = 'show'
                    } else {
                        data[attr].url = '';
            ...
            //设置录播-直播值
            if (Home.Data.sysmode == 'buildSelf') {//自建模式：会议录播
            ...
            if (Home.Data.sysmode == 'rent') {//租赁：会议直播
            ...
            for (var i = 0; i < moduleData[Home.Data.domainType].modules.length; i++) {
                if (moduleData[Home.Data.domainType].modules[i].show != 'hidden') {
                    moduleData[Home.Data.domainType].width = moduleData[Home.Data.domainType].width + 163;
                }
            }
            return moduleData[Home.Data.domainType];
        };
        let headerModule = that.processData(modules);
        i18nUtil(renderHeaderModules, headerModule, '.module-entry-wrapper');
    },
    initMainContent: function (panelData) {
        let that = this;
        this.initMainContentTemplate = function () {
            if ('mooooooo-oooo-oooo-oooo-bmcdebugger' === Home.Data.userMoid || 'mooooooo-oooo-oooo-oooo-bmcdeveloper' === Home.Data.userMoid) {
                panelData = {};
                return panelData;
            }
            if (Home.Data.isServiceDomainAdmin && !Home.Data.isSimpleMcu) {//服务域管理员
            ...
            if (Home.Data.isUserDomainAdmin) {//用户域管理员情况下显示的内容
                ...
                if (Home.Data.domainType == 'coreDomain') {
            ...
            if (Home.Data.isUsualUser) {//一般用户
                ...
                if ('coreDomain' === Home.Data.domainType && panelData.meeting_info) {
            ...
            } else if ('coreDomain' === Home.Data.domainType) {
                panelData['meeting_info']['head_more'].shift(); //核心域不显示创建会议
            }
            return panelData;
        };
        let data = that.initMainContentTemplate();
        for (let attr in data) {
            let contentModule = data[attr];
            i18nUtil(renderContentModules, contentModule, '#row');
        }
        console.log(panelData);
        this.panels = initPanels.init(panelData);
...
$.namespace("initPanels");
initPanels = {//此处更新
    init: function (panelData) {
        var initResourceLoad = this.initResourceLoad(panelData);
    ...
    initResourceLoad: function (panelData) {
        ...
        if (panelData.hasOwnProperty("resource_load")) {//是否有资源负载，有则显示并加载
            var resourceLoadPanel = new InitPanel(function () {//初始化无数据
                i18nUtil(renderResourceModules, panelData.resource_load, '#resource_load');
            }, function () {//ajax获取数据
