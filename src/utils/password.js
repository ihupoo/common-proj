...
Mo.Password = {
			...
			if($this.next().hasClass("showHidePassword")){
				if(!$this.next().hasClass("click")){
					...
					if($(that.newPasswordId).next().hasClass("showHidePassword")){
						...
						var newPassword = $.trim($(Mo.Password.newPasswordId).val());
						...
						//判断密码是否使用过
						if(Mo.Password.checkUsed){
							var oldPassword = $.trim($(Mo.Password.oldPasswordId).val());
							var cdata = {};
							if(!!oldPassword){
								if(newPassword.length>0){
                                    if('1' === systemSecurity){
                                        cdata = Mo.Base.DigestAuth.makePassword(oldPassword,newPassword);
                                    }else{
                                        cdata = Mo.Base.DigestAuth.makePassword(hex_md5(oldPassword),newPassword);
                                    }
                                    $.post(Mo.Config.appUrl + '/system/user/checkNewPassword', cdata, function (msg) {
                                        if (msg.success){
                                            return Mo.Password.checkNewPasswordCallback(msg.data);
                                        }
                                    }, 'json').fail(function () {
                                    });
								}
								...
								cdata = Mo.Base.DigestAuth.makePassword(nonce,newPassword);
	...
	checkNewPasswordCallback: function(data){
		if (data === '1') { //旧密码使用过
			Mo.Password.clearNewPasswordTip();
			Mo.Password.showTips($(Mo.Password.newPasswordTip), false, "密码已使用");
			$('.newPasswordTip .help').hide();
			$(Mo.Password.newPasswordId).focus();
			return false;
		}
		return true;
	},
		...
		checkPassword:function(newPass){//检测旧（当前）密码是否正确
			...
			if(oldPassword!=""&&oldPassword!=undefined){
				...
				var data = {};
				if('1'==systemSecurity){
					data = Mo.Base.DigestAuth.makePassword(oldPassword,oldPassword);
				}else{
					data = Mo.Base.DigestAuth.makePassword(hex_md5(oldPassword),oldPassword);
				}
				data.username = Mo.Base.DigestAuth.username;
		...
		showStrength:function(password){
			...
			if(Mo.Base.account.getPasswordStrength(password)=="weak"){
			    $(this.newPasswordTip).find('.tip-icon').toggleClass("weak-icon");
			    $(this.newPasswordTip).find('.tip-info').toggleClass("weak-tip");
			    $(this.newPasswordTip).find('.tip-info').text("弱");
			}
			if(Mo.Base.account.getPasswordStrength(password)=="medium"){
			...
			if(Mo.Base.account.getPasswordStrength(password)=="strong"){
			...
			if(Mo.Base.account.getPasswordStrength(password)=="fault"){
