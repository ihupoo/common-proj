var controller={
    ...
    getEightConferenceInfo:function($this){//获取八方会议
    	...
    	var data = {item_resource_data:this.eightData};
	        ...
	        for(var i = 0; i<data.item_resource_data.length;i++){
	            ...
	            // var usedPersent = item.total==0?0:item.used/item.total;
	            // usedPersent = Math.round(usedPersent*100)
        ...
        var requestGet = $.get(url,{'moid':moid,'start_time':start_time,'end_time':end_time},function(t){
			if(t.success){
				...
				for(var i = 0;i<data['time'].length;i++){
					...
					// data['time'][i] = data['time'][i].slice(0,-3)
				...
				// $(".maxValue","#"+bookMeetingCountData.contentId).text(data.max);
				// $(".minValue","#"+bookMeetingCountData.contentId).text(data.min);
				// $(".averageValue","#"+bookMeetingCountData.contentId).text(data.average);
    ...
    getLiveRoomInfo:function () {//获取直播室信息
        //var url = data['vrs_live'].url+"/api/v1/vrs/liveroom?userdomainmoid="+userDomainMoid+"&prgs1page=10&pageid=1&includename=&orderindex=7&desc=1&sso_token=" + sso_cookie;
